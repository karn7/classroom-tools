...
function getUnlockKey(courseId: string) {
  return `classroom-tools:course-unlocked:${courseId}`;
}

export function unlockCourseSession(courseId: string) {
  window.sessionStorage.setItem(getUnlockKey(courseId), "true");
}

export function isCourseUnlocked(courseId: string) {
  return window.sessionStorage.getItem(getUnlockKey(courseId)) === "true";
}

export default function CourseAccess({ children, courseId }: CourseAccessProps) {
  ...
  if (!hasLoaded) {
    return <div className="stage-viewport bg-[#f6f7fb]" />;
  }
