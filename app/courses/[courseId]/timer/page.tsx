...
import TimerTool from "@/app/timer/TimerTool";
import CourseAccess from "@/components/CourseAccess";

export default async function CourseTimerPage({
  params,
}: {
    ...
    <CourseAccess courseId={courseId}>
      ...
      <main className="stage-16-9 flex flex-col gap-4 p-4 sm:p-5">
        <header className="flex shrink-0 flex-col gap-3 rounded-[1.5rem] bg-white px-5 py-4 shadow-sm ring-1 ring-slate-200 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-base font-semibold text-indigo-700">กิจกรรม</p>
          ...
          <Link
            ...
            className="inline-flex min-h-14 items-center justify-center rounded-2xl bg-slate-950 px-6 text-xl font-bold text-white shadow-sm transition hover:bg-slate-800 focus:outline-none focus:ring-4 focus:ring-indigo-200"
          >
            กลับหน้าหลัก
          </Link>
        </header>
